export const BEERS = [
  { id: "B1", name: "Barfüßer", price: "3,80 EUR" },
  { id: "B2", name: "Frydenlund", price: "150 NOK" },
  { id: "B3", name: "Grieskirchner", price: "3,20 EUR" },
  { id: "B4", name: "Tuborg", price: "5,50 EUR" },
  { id: "B5", name: "Baltic Tripple", price: "6,95 EUR" },
  { id: "B6", name: "Viktoria Bier", price: "4,20 EUR" }
];

export const RATINGS = [
  {
    id: "R1",
    beerId: "B1",
    author: "Waldemar Vasu",
    comment: "Exceptional!"
  },
  {
    id: "R2",
    beerId: "B2",
    author: "Andrea Gouyen",
    comment: "Very good!"
  },
  {
    id: "R3",
    beerId: "B3",
    author: "Allan Venkat",
    comment: "Great taste!"
  },
  {
    id: "R4",
    beerId: "B4",
    author: "Nebuchadnezzar Voitto",
    comment: "Try it, you'll love it!"
  },
  {
    id: "R5",
    beerId: "B5",
    author: "Hevel Nubia",
    comment: "My favorite!"
  },
  {
    id: "R6",
    beerId: "B6",
    author: "Karl Marx",
    comment: "✊"
  },
  {
    id: "R7",
    beerId: "B1",
    author: "Madhukar Kareem",
    comment: "Awwwesome!"
  },
  {
    id: "R8",
    beerId: "B2",
    author: "Marketta Glaukos",
    comment: "phenomenal!"
  },
  { id: "R9", beerId: "B1", author: "Cecilija Viktoria", comment: "Can I order another please?" },
  {
    id: "R10",
    beerId: "B3",
    author: "Ismene Yehonatan",
    comment: "Tastes moreish"
  },
  {
    id: "R11",
    beerId: "B4",
    author: "Seong-Hun Cihangir",
    comment: "Hmmmm!!!!"
  },
  {
    id: "R12",
    beerId: "B5",
    author: "Guillelmina O'Doherty",
    comment: "Watery mouthfeel and long finish."
  },
  {
    id: "R13",
    beerId: "B6",
    author: "Alessa Bradley",
    comment: "Dense, dense mouthfeel and syrupy finish"
  },
  {
    id: "R14",
    beerId: "B1",
    author: "Emily Davis",
    comment: "Off-putting buttery nose, laced with a touch of caramel and hamster cage."
  },
  {
    id: "R15",
    beerId: "B2",
    author: "Lauren Jones",
    comment: "Delicate buttery flavor, with notes of sherry and old newsprint. "
  }
];
